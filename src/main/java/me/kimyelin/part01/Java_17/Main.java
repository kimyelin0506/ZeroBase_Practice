package me.kimyelin.part01.Java_17;// Java 프로그래밍 - 스트림
...
//      1-1. 배열 스트림
        ...
        Stream stream1 = Arrays.stream(arr); //stream 변환
...
//        stream2.forEach(System.out::println);
        stream2.forEach(n -> {  //최종 연산을 만나면 종료 처리하기때문에
...
//      1-4. 스트림 generate
        ...
        Stream streamGenerate = Stream.generate( () -> "abc").limit(3);  //3만큼 반복
...
//      2-1. Filtering
        ...
        intStream2.forEach(n-> System.out.println("n="+n));
...
//      2-3. Sorting
        ...
        IntStream intStream4 = IntStream.builder().add(1).add(4).add(2).add(3).add(5).build();
        intStream4 = intStream4.sorted();
...
//      3-1. Sum, Average
        ...
        int num = IntStream.range(1,5).sum();
...
//      3-3. reduce
        ...
        Stream<Integer> stream3 =
                new ArrayList<Integer>(Arrays.asList(1,2,3,4,5)).stream();
        System.out.println(stream3.reduce((x,y) -> x+y).get());  //연쇄적
